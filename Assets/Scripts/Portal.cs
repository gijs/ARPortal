...
public class Portal : MonoBehaviour
{
	...
	void OnWillRenderObject () {
		...
		if (!leftTexture) {
			...
			renderCam = Instantiate (mainCamera); // Make an exact copy to support ARKit
			...
			if (renderCam.GetComponent<AudioListener> ()) {
				Destroy (renderCam.GetComponent<AudioListener> ());
			}
			if (renderCam.GetComponent<FlareLayer> ()) {
				Destroy (renderCam.GetComponent<FlareLayer> ());
			}
			if (renderCam.GetComponent<GUILayer> ()) {
				Destroy (renderCam.GetComponent<GUILayer> ());
			}

			// This keeps AR Tracking from getting buggy
			foreach (Component component in renderCam.GetComponents<Component>()) {
				if (component.name.Contains("Unity AR Camera")) {
					Destroy (component);
				}
			}
